function [beta, order, count, attributes,model_preps,R,rms] = SKPLS(X, y, A,yadd,plotting)
% swiss kinfe PLS
...
for a = 1:A 
    ...
    for k=1:nb 
        if ndims(X{1,k})>2         % extracting scores
             ...
             [temp_v,~] = Ra(E, Y, yprim, []); 
             %temp_v = E'*yprim(:,inddd);
             Zz=reshape(temp_v,dims(:,2:end));
	         [wjx{k},~,wkx{k}] = svds(Zz, 1);
             t(:,k) = E*kron(wkx{k},wjx{k});
             %ncomp = wrt(E,yprim,kron(wkx{k},wjx{k}));  % i have this idea
             %for extracting only signifacnt components to avoid
             %cross-validation analysis, but maybe next paper
             %signicomp(a,k) = ncomp;
...
%     else
%     [~,i] = min(sum(cell2mat(r).^2));  % for single response use the residual for winner
%     end
