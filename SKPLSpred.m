function [R,rmsep] = SKPLSpred(Xt,yt,Xc,yc,beta,preps,plotting)
% Fucngtion to use the SKPLS model to predict on new data
% Xt : test data in form of cell
% Yt : test response
% Xc : calibration data used for training model
% Yc : calibration response used for training model
% beta : beta coefficient from SKPLS model
% preps : pre-processing used for calibraiton set
% plottitng : 1 or 0 to have plotting or not
nb = length(preps);
n = size(Xt{1,1},1);
...
if plotting==1
    ...
    plot(yt,([zeros(size(yt,1),1) cell2mat(Xt)]*beta(:,end))+mean(yt),'^r');xlabel(['Measured ']);ylabel(['Predicted']);
    lsline;
